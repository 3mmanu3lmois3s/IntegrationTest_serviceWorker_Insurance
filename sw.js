...
self.addEventListener('install', (event) => {
  ...
  event.waitUntil(self.skipWaiting()); // Force the new SW to become active
});

self.addEventListener('activate', (event) => {
  console.log('Service Worker activating.');
  event.waitUntil(
    clients.claim().then(() => { // Claim clients immediately
      openDB(); // Open IndexedDB *after* claiming
      testHttpMethods();  // Now safe to run tests
    })
  );
});
...
self.addEventListener('fetch', (event) => {
  ...
  if (requestUrl.pathname.startsWith(basePath)) {
    ...
    if (relativePath === 'products' && method === 'GET') {
        ...
        // Handle API requests.  Prioritize /test routes.
        ...
        if (relativePath.startsWith('api/')) {
            ...
            switch (apiPath) {
                ...
                case 'users':
                    ...
                    break;
                case 'customers':  //Handles both, post and get all customers
                    if(method === 'POST'){
